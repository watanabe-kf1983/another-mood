...
class QueryNode(Protocol):
    ...
    def derive(self, catalog: dc.CatalogNode) -> dc.CatalogNode:
...
class SelectItem:
    ...
    def derive(self, catalog: dc.CatalogNode) -> tuple[dc.CatalogEdge, dc.CatalogNode]:
...
class Select:
    ...
    def derive(self, catalog: dc.CatalogNode) -> dc.CatalogNode:
        return dc.CatalogNode(children=[item.derive(catalog) for item in self.items])
    ...
    def derive(self, catalog: dc.CatalogNode) -> dc.CatalogNode:
        return dc.CatalogNode(
            children=[
                ...
                (
                    dc.CatalogEdge(name=self.as_name, type="object[]", required=True),
                    catalog,
                ),
            ],
        )
