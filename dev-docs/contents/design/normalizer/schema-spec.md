...
- **Entity**: データツリー上の到達経路 (`access_path`) を持つ。クエリ DSL の `from:` や URL/anchor、ファイルパス、表示見出しに使う。例: `categories`, `categories.tasks`
- **ObjectType**: row 型そのものの識別子 (`id`)。コレクションを 1 段降りるたびに `.item` を付加する path-based 名。FK 参照や型レベルの cross-reference に使う。例: `categories.item`, `categories.item.tasks.item`

```yaml
properties:
  categories:                # access_path: "categories"
    type: object             # object_type.id: "categories.item"
    additionalProperties:
      type: object
      properties:
        tasks:               # access_path: "categories.tasks"
          type: object       # object_type.id: "categories.item.tasks.item"
...
ObjectType に対する人間向けの表示名 (例: `Category`) を `title:` キーワードで指定する仕組みは別タスク (M4) で導入予定。
