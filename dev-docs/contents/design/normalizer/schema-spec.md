...
> **進行中** — D1 (meta-schema 受理) と D2 (Schema-Inspector による収集とデータカタログ反映) は実装済み。残る Phase 10 作業は [D3〜D5](../../../tasks.md) (schema-level コヒーレンスチェック、showcase 適用、docs 同期)。data-level の FK 整合性検査と警告インフラ ([D6, D7](../../../tasks.md)) は Phase 11+ に温存。Unique 制約 (D8, D9) は別記、[normalizer.md](normalizer.md) を参照。
...
Schema-Inspector は traversal 中に x-ref を `Attribute.x_ref: XRef | None` (data_catalog.py) として収集する。`Attribute.catalog` (catalog の自己記述 Node) にも `x_ref` / `x_ref.entity` / `x_ref.attribute` の三辺を追加してあるため、DSL からは `attr.x_ref.entity` 等で参照できる。これで D6 の data-level 検証も F4/F5 のリレーション描画も、同じカタログを単一の真実として消費する形を取れる。
...
- ~~**D1**~~ (実装済み): Meta-schema 拡張 — `x-ref` を property 直下で受理。`items:` 直下は明示エラー
- ~~**D2**~~ (実装済み): Schema-Inspector 拡張 — traversal 中に x-ref を収集して `Attribute.x_ref` に反映
