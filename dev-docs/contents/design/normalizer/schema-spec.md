...
- **Entity**: データツリー上の到達経路を表す identifier (`id` = access path)。クエリ DSL の `from:`、paging 設定、表示見出しに使う。例: `categories`, `categories.tasks`。アンカー（リンク用の実体パス）とは別概念で、こちらは [anchor-spec.md](../generator/anchor-spec.md) を参照
