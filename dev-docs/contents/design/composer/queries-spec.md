...
> **未実装** — Phase 10 タスク [E6](../../../tasks.md)。現状は Composer が正規化済みデータと同名のクエリを silent に上書きする。
...
> **未実装（親参照）** — Phase 10 タスク [M1](../../../tasks.md)。`_parent` の付与自体が未実装のため、以下の例の `task._parent.title` は現時点では空文字としてレンダリングされる。`from` のドット記法によるフラット化自体は動作する。
