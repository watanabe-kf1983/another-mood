...
`flatten: { of: tasks, as: task }` の出力は `{ id, title, task: { ... } }` のように **親 fields を top-level に残し、 child を `as:` 名の namespace 配下に置く** 形にした。旧 `from: a.b` (PR 2 で廃止予定) のように child を top-level に昇格させて親情報を失わせる方式は採らない。
...
E1 (where) / E2 (sort) に続く DSL 拡充前半 (E11) の後段。flatten 句と走査の非対称性 (External Design 節参照) は実装済みで、 残るのは `from:` のドット記法廃止。 Phase 10 タスク [E11](../../../tasks.md)。 E3 (join 導入) の前提。
...
E8 で導入した longest-match walk は、 composition walk を `flatten:` 句に分離することで不要になる。 `From.derive` は `catalog.child(name)` 単発 lookup に簡略化される (`walk_path` は他用途で引き続き使用)。
...
`walk_path` 本体は引き続き残るが、 他の句 (`sort.by:` / `where` 等) は `has_child` による direct edge lookup を採用しており、 walk_path には依存しない (走査の非対称性ルール参照)。 array 跨ぎの dot path はそもそも有効な edge にならない (catalog 上、 array element の attribute は親 entity の sibling edge ではない) ため、 has_child で自然に弾かれる。
