...
- `__meta_query/{id}.md` — クエリの Definition + Output Schema + Results
  (F3 で Output Schema / Results を追加予定)
...
| Query | `__meta_query` に Definition / Output Schema / Results の **3 セクション 1 ページ** |
...
**Query** は Definition / Output Schema / Results が 1 つの recipe 由来の束:

- Definition: user が書いた DSL
- Output Schema: Definition から deterministic に推論 (E7 の `apply_to_catalog`)
- Results: Definition + 上流データから生成
- どれも単独では不完全: Definition は抽象 recipe、Schema は「結果の形」単独で
