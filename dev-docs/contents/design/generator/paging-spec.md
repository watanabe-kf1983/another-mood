...
これは [分割ルール](#分割ルール) の two-loop（`| link` + `{% mood_view %}`）と `under_heading` を、フォルダ木に一般化しただけ。`order_key` / `depth` は id のみに依存する純導出なので、generator フィルタではなく **prose の preprocess**（`prose.py` の `_outline_position`）で prose レコードのフィールドとして供給する（`title` の隣、per-file で完結、単体テストも preprocess 側）。preprocess の出力はファイル単位でソートできないので、**ソート（`sort(attribute="order_key")`）は collection の揃うテンプレ側**に置く。

#### 決定

- **① グルーピング**: 上の flat-loop 方式。`order_key` / `depth` は preprocess（`prose.py`）で prose レコードに導出（純関数 + 単体テスト、`components/` カバレッジ対象）、ソートはテンプレの組込 `sort`。再帰マクロや prose のフォルダノード化（[リンク解決の単一ページ不変条件](generator.md#描画の単一ページ不変条件-mood_view-subtree-ガード) 参照。現状 prose は `/prose` 直下のフラット）は重いので採らない（YAGNI・実需は深さ計算だけ）。
