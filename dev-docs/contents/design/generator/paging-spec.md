...
> 残タスク（Phase 11/13）: 複数プロファイル (C5/C6)、mood_view 自動アンカー刻印 (C9、[anchor-spec.md](anchor-spec.md#mood_view-自動アンカー刻印)。受け側 `| anchor` の B9 は実装済み)、meta 出力ディレクトリの集約 (F9)、prose body フィルタ (B5)。
