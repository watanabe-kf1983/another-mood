...
> 残タスク: Editions (C5/C6)、meta 出力ディレクトリの集約 (F9)。
...
edition の出力は **`{outDir}/{edition}/{page_path}`**。`reports/` ラッパーは持たない — report の中身は edition ディレクトリ直下に出て、`{outDir}` 直下の診断系出力（`index.md`・`__entity_defs/` 等）と横並びになる:

```
{outDir}/
  index.md                            ← メタ index（入口。各 edition を列挙）
  default/   (form B なら web/ pdf/)      ← edition（report 本体）
  __entity_defs/  __entity_data/  __queries/   ← 診断（edition 横断）
```

診断系は edition 横断で常に同じ位置に出る（`__` 予約済みなので edition 名と衝突しない）。
...
> **背景: 現行 form A の出力も `reports/` → `default/` に動く.** これは C1–C4 で実装済みの単一 report 出力（`{outDir}/reports/`）を変える破壊的変更。単一 report を「1 edition の特殊形」に統一する帰結で、固定ラッパー `reports/` は廃される。

#### メタ index の Reports セクション

メタ index (`{outDir}/index.md`) の `## Reports` は各 edition を名前でリンクする（`[default](default/)`、form B なら `[web](web/)`・`[pdf](pdf/)`）。メタ render はユーザ config（`reports.yaml`）を持たない（内部の `META_EDITION` で回る）ため、列挙には edition 名一覧をメタ render のコンテキストに配線する。edition 横断の landing ページは作らない — 入口はメタ index に集約する。

#### Edition と逐次生成 (C6)

ローダ `load_editions(reports_file)` は form A/B を吸収し、必ず `Sequence[Edition]` を返す:

- form A → `[Edition(name="default", file_per=[...])]`
- form B → `[Edition(name="web", ...), Edition(name="pdf", ...)]`

`generate()` はこの列を**逐次**ループし、各 `edition` を `{outDir}/{edition.name}` へ render する（自分の `Edition` と、それに束ねた `make_link_filters(edition, node_map)` を使う）。"並列ビルド" (C6) は成果物が横並びに出る意で、concurrency は持ち込まない（必要が出た時点で別途）。単一 edition 選択（`--edition`）も当面持たない。
...
将来、edition ごとに異なるルートテンプレート（`index.md` 以外）を指定したい需要がある（Web 版と印刷版でトップ構成を変える等）。これは `file_per` と同じ `Edition` の継ぎ目に `root_template` 等のフィールドを足し、`generate()` のループ本体で render するルート名を差し替えるだけで乗る加算的拡張で、本タスク (C5/C6) のスコープからは外す。継ぎ目を保つため、ルートテンプレート名 `"index.md"` を `generate()` の奥にハードコードし続けず `Edition` 由来にできる形を意識する（現状は `"index.md"` 固定）。
