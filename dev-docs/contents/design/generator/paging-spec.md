...
### dev-docs book edition のテンプレ構造

> **未実装** — dev-docs を web / book 両 edition で正しく出すためのテンプレ移行タスク。**正しさ → 順序** の 2 段で進める。順序段の番号 / id 方針は [prose の章順と id の関係](../normalizer/markdown-parser-spec.md#prose-の章順と-id-の関係) が正本。本節はテンプレ構造側の方針とステップの記録。

現 dev-docs のルートテンプレは prose を id 辞書順でフラットに全 inline し tasks / roadmap を続けるだけで、book edition（全 inline）では H1 が乱立しフォルダの親子（`design` → `generator` → …）がネストされない。狙いは music showcase（two-loop + `under_heading`、`reports.yaml` の form B）と同じレイヤリングを、dev-docs の深い prose 木に適用すること。

#### 鍵: グルーピングは sort ではなく構造で

「順序は暫定でもネスト構造は正しい」を成立させるには、グルーピングを sort 依存にできない — id 辞書順は各フォルダの `index` を部分木の中程に落とし、サブフォルダとファイルを混ぜる。ただし再帰も tree-builder も要らず、**フラットな 1 ループ + ノード毎の見出しレベル規則 + folder-preorder ソートキー**で足りる:

- **folder-preorder ソートキー（`order_key`）**: `…/index` を所属フォルダ `…/`（末尾スラッシュ）へ写像してから文字列ソートする。各フォルダの `index` が兄弟の先頭に来て、部分木が連続する pre-order 列になる。順序段の番号でも Phase 1 の暫定英字でも、同じキーがそのまま効く。
- **見出しレベル規則（`depth`）**: `depth = セグメント数`（`index` のとき）/ `セグメント数 + 1`（非 `index`）。→ トップ `index` = `#`、章 `index` = `##`、章直下ファイル = `###`、サブ章 `index` = `###`、その下 = `####`。各ノードを `under_heading("#" * (depth-1))` で包む（`under_heading` は本文 H1 を marker+1 段へ落とすので、title を `H{depth}` に載せる marker は `depth-1`。トップ `index` は `depth 1` = shift 0 なので un-wrap）。split 時は `{% mood_view %}` が `""` を返すので web は無影響。

これは [分割ルール](#分割ルール) の two-loop（`| link` + `{% mood_view %}`）と `under_heading` を、フォルダ木に一般化しただけ。`order_key` / `depth` は id のみに依存する純導出なので、generator フィルタではなく **prose の preprocess**（[prose.py](../../../../src/another_mood/components/preprocess/prose.py) の `_outline_position`）で prose レコードのフィールドとして供給する（`title` の隣、per-file で完結、単体テストも preprocess 側）。preprocess の出力はファイル単位でソートできないので、**ソート（`sort(attribute="order_key")`）は collection の揃うテンプレ側**に置く。

#### 決定

- **① グルーピング**: 上の flat-loop 方式。`order_key` / `depth` は preprocess（[prose.py](../../../../src/another_mood/components/preprocess/prose.py)）で prose レコードに導出（純関数 + 単体テスト、`components/` カバレッジ対象）、ソートはテンプレの組込 `sort`。再帰マクロや prose のフォルダノード化（[リンク解決の単一ページ不変条件](generator.md#描画の単一ページ不変条件-mood_view-subtree-ガード) 参照。現状 prose は `/prose` 直下のフラット）は重いので採らない（YAGNI・実需は深さ計算だけ）。
- **② フォルダ intro の統一**: 全フォルダが `index.md`（節の前書き＝節見出し）を持つ形に揃える。現状 `index.md` が無い `background/` `dev/` と design 配下 4 サブフォルダ（`app/` `composer/` `generator/` `normalizer/`）に新設する（計 6）。既存 `index.md` の**手書き ToC リンク列と子セクション見出しは撤去**し、そのフォルダ自身の前書き散文だけ残す（ナビ・子見出しは folder-loop が生成する）。`index` は content のまま置く（[order/id Proposal の「却下した代替案」](../normalizer/markdown-parser-spec.md#prose-の章順と-id-の関係) どおり、住所性を保つため template 層へは移さない）。
- **③ `[→]` proposal リンクの暫定**: `tasks.md` / `roadmap.md` の `[→](prose/…#見出し)` は見出しノード（[A1](node:/tasks/A/tasks/A1) / [A7](node:/tasks/A/tasks/A7)）未実装のため book で追従できない。暫定で**ページ単位 node() リンクに落とす**（見出し精度は捨てる。ページ単位 `node(path="/prose/…") | link` は既に両 edition 追従する）。見出し精度は A7 で回復する。

#### ステップ

進捗は各ステップ＝ 1 コミットとし、`git log` を正本にする（本節に done 印は持たない）。再開時は `mood build dev-docs` → 本 Proposal → `git log` の順で拾う。

**Phase 1 — 正しさ（1 PR、順序は暫定英字）**

1. ベースライン: `make ci` + 現 form A ビルドを diff 基準に取る
2. form B 化: `reports.yaml` を `editions: {web: 分割, book: []}` に。web ≈ 旧 default を確認、book を目視して壊れの実物を出す
3. グルーピング方式のスパイク（鍵の flat-loop を最小検証）＋ ①②③ 最終確認
4. コンテンツ前提: 6 フォルダ（`background/` `dev/` + design 配下 4）に `index.md` を新設、既存 `index.md` の手書き ToC・子見出しを撤去（②）
5. `order_key` / `depth` を preprocess（prose.py）で導出 + 単体テスト（①）
6. ルートテンプレ書き換え: nav(ToC) ループ + body 埋め込みループの two-loop、`under_heading` 深さネスト、末尾空行の block 分離、末尾に tasks / roadmap 章
7. `[→]` proposal リンクをページ単位 node() に（③）
8. 検証: 両 edition ビルド、B12 subtree ガード違反ゼロ、prose 本文 `node:` 解決（web = 相対 / book = fragment）、出力を未解決 `node:` で grep、必要なら `mood watch` + Playwright
9. docs / ポインタ同期: `DEVELOPMENT.md` / `CLAUDE.md` の入口パス更新（出力構造が変わる）、`make ci` → PR

**Phase 2 — 順序（別 PR）**

10. 番号規約の確定（桁・隙間の刻み・ディレクトリにも番号・`index` は無番号。[order/id Proposal](../normalizer/markdown-parser-spec.md#prose-の章順と-id-の関係)）
11. リネーム + 参照貼り直し: ファイル / ディレクトリに番号付与（エディタの rename 追従で相対 `.md` リンクは自動更新、`tasks.yaml` の `spec:` / `proposal:` は手動）。フォルダ単位でコミット
12. 順序検証: 再ビルド、未解決 `node:` grep、両 edition で読む順を確認、**テンプレ無改修で番号が効く**ことを確認（`order_key` は Phase 1 で folder-preorder 済み）、`make ci` → PR

完了時は本節を trim する（残る設計判断は Internal Design へ移し、ステップ表は消す）。
