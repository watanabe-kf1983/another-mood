...
`file_per` にはページとして切り出す対象 ObjectType ID ([schema-spec.md](../normalizer/schema-spec.md#entity-名) 参照 — 実行時には各ノードの `_meta.object_type_id` ([generator.md](generator.md#ノードメタデータ-b1-b3-b6)) と照合される) を列挙する:
...
`{% mood_view "tpl" with NODE %}` は主題ノードの `_meta.object_type_id`（[generator.md](generator.md#ノードメタデータ-b1-b3-b6)）を `file_per` と照合して振る舞いを決める:
...
> **注: インライン展開時の fragment 自動着地は未完（C9）.** `| link` のインライン解決は `#anchor_path` を指す。受け側 `<a id>` を置く `| anchor` フィルタは実装済み（[anchor-spec.md のリンク記法](anchor-spec.md#リンク記法)、B9）だが、mood_view が描画ノードに自動で刻む段（[C9](anchor-spec.md#mood_view-自動アンカー刻印-c9)）は未実装。それまで着地点は author が `| anchor` で手置きした位置のみ。two-loop の PDF 体験（同ページ内ジャンプ）が手置き不要で完成するのは C9 まで行ってから。分割側のページ単位リンクは path 部で機能する。
...
> 残タスク（Phase 11/13）: 複数プロファイル (C5/C6)、mood_view 自動アンカー刻印 (C9、[anchor-spec.md](anchor-spec.md#mood_view-自動アンカー刻印-c9)。受け側 `| anchor` の B9 は実装済み)、meta 出力ディレクトリの集約 (F9)、prose body フィルタ (B5)。
