...
- **folder-preorder ソートキー**: `…/index` を所属フォルダ `…/`（末尾スラッシュ）へ写像してから文字列ソートする。各フォルダの `index` が兄弟の先頭に来て、部分木が連続する pre-order 列になる。順序段の番号でも Phase 1 の暫定英字でも、同じキーがそのまま効く。
- **見出しレベル規則**: `level = セグメント数`（`index` のとき）/ `セグメント数 + 1`（非 `index`）。→ トップ `index` = `#`、章 `index` = `##`、章直下ファイル = `###`、サブ章 `index` = `###`、その下 = `####`。各ノードを `under_heading("#" * level)` で包む（split 時は `{% mood_view %}` が `""` を返すので web は無影響）。

これは [分割ルール](#分割ルール) の two-loop（`| link` + `{% mood_view %}`）と `under_heading` を、フォルダ木に一般化しただけ。ソートキー / level は小さな純関数なので、テンプレ inline より generator フィルタとして持てば単体テストできる（[data_tree_filters.py](../../../../src/another_mood/components/generator/data_tree_filters.py) の隣）。

#### 決定

- **① グルーピング**: 上の flat-loop 方式。ヘルパは sort key / level の小フィルタ（単体テスト必須、`components/` カバレッジ対象）。再帰マクロや prose のフォルダノード化（[リンク解決の単一ページ不変条件](generator.md#描画の単一ページ不変条件-mood_view-subtree-ガード) 参照。現状 prose は `/prose` 直下のフラット）は重いので採らない（YAGNI・実需はレベル計算だけ）。
- **② フォルダ intro の統一**: 全フォルダが `index.md`（節の前書き＝節見出し）を持つ形に揃える。現状 `index.md` が無い `background/` `dev/` に新設する。既存 `index.md` の**手書き ToC リンク列と子セクション見出しは撤去**し、そのフォルダ自身の前書き散文だけ残す（ナビ・子見出しは folder-loop が生成する）。`index` は content のまま置く（[order/id Proposal の「却下した代替案」](../normalizer/markdown-parser-spec.md#prose-の章順と-id-の関係) どおり、住所性を保つため template 層へは移さない）。
...
4. コンテンツ前提: `background/index.md` `dev/index.md` を新設、既存 `index.md` の手書き ToC・子見出しを撤去（②）
5. ヘルパ + 単体テスト: sort key / level フィルタ（①）
...
12. 順序検証: 再ビルド、未解決 `node:` grep、両 edition で読む順を確認、**テンプレ無改修で番号が効く**ことを確認（sort key は Phase 1 で folder-preorder 済み）、`make ci` → PR
