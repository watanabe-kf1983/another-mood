...
リンク解決は pre-render 段階で完結する (post-render の文字列置換は採らない)。各テンプレート render 用の TemplateEngine インスタンスに対し、resolver を closure binding で渡し、各種フィルタが共有する。source ページ側の URL は B6 が注入する `_meta.page_url` を `@pass_context` 経由で ctx から読む形で、TemplateEngine 側に専用の配管は持たせない (詳細は [anchor-spec.md#リンク解決](anchor-spec.md#リンク解決))。
