...
| `_meta.anchor_id` | 全ノード | このノードのアンカー ID 文字列 ([anchor-spec.md](anchor-spec.md) の規則に従う)。root は空文字列 |
| `_meta.object_type_id` | 全ノード | このノードのスキーマ位置を表す path 形 ID。Mapping は ObjectType ID (`X.item`)、Array は field path 形 (`X.item.yyy`、catalog 表記 `X.item.yyy.item[]` の同義)。root は空文字列 |

対象範囲は anchor ID 付与可能な全 Mapping / Array ノード ([anchor-spec.md](anchor-spec.md) 参照)。リスト要素に `id` フィールドが無い場合はその要素も配下もアンカー ID を持たないため、ラップ対象から除外する (= 親参照を持たない素の dict / list として残る)。Array 配下の Array も同様にラップ対象外 — アンカー上の位置を表す手段がないため、生 list として通す。
...
> **未実装** — Phase 11 タスク [B6](../../../tasks.md)。`_meta.anchor_id` / `_meta.object_type_id` (B3) は実装済 — [ノードメタデータ](#%E3%83%8E%E3%83%BC%E3%83%89%E3%83%A1%E3%82%BF%E3%83%87%E3%83%BC%E3%82%BF) 参照。

`_meta` に表示先 URL を追加する。

| フィールド | 対象ノード | 内容 |
|---|---|---|
| `_meta.page_url` | 全ノード | 表示先 URL。file_per 設定 ([paging-spec.md](paging-spec.md)) を参照し、自身が分割対象なら自ページ、でなければ親側を遡って解決 |

実装方針: B3 同様 `_parent` チェインを辿る lazy 評価。判定には `_meta.object_type_id` と paging 設定を引き合わせる。

### リンク解決 (B2, B4, B5)

> **未実装** — Phase 11 タスク [B2, B4, B5](../../../tasks.md)。アンカーの仕様 (リンク記法 / フィルタ API / 解決のタイミング) は [anchor-spec.md](anchor-spec.md) を参照。前提となる TemplateEngine 側の配管 (render 経路の統合 / `current_source()` を露出する source-page スタック) は実装済み — [template_engine.py](../../../../src/another_mood/components/generator/template_engine.py) 参照。

リンク解決は pre-render 段階で完結する (post-render の文字列置換は採らない)。各テンプレート render 用の TemplateEngine インスタンスに対し、resolver を closure binding で渡し、各種フィルタが共有する。source ページの URL は `TemplateEngine.current_source()` を見る (URL 形式と相対パス算出は [anchor-spec.md#リンク解決](anchor-spec.md#リンク解決) 参照)。

#### anchor_id → ノード マップ (B2)

anchor 系フィルタ (B4) と resolve フィルタ (B5) は anchor ID からノードを引く必要がある。B3 が lazy 評価のため、別途 1 度のツリー走査で anchor_id → ノードのマップを構築する。
