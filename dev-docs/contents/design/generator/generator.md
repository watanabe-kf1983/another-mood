...
- **`iter_nodes` は free function**。`_` プレフィックスはノードのテンプレート公開フィールド専用 ([data_tree.py](../../../../src/another_mood/components/generator/data_tree.py) 冒頭参照) なので、内部専用の走査をノードのメソッドにすると命名規約と衝突する。`build_anchor_map` の走査 1 回はツリー全体を舐める他処理 (B6 の `page_path` 事前計算等) の共通基盤にも転用できる
...
> **未実装** — Phase 11 タスク [B6](../../../tasks.md)。`_meta.anchor_path` / `_meta.object_type_id` (B3) は実装済 — [ノードメタデータ](#%E3%83%8E%E3%83%BC%E3%83%89%E3%83%A1%E3%82%BF%E3%83%87%E3%83%BC%E3%82%BF) 参照。

`_meta` に **表示先ページのパス** (レポートルート相対、fragment 抜き) を追加する。

| フィールド | 対象ノード | 内容 |
|---|---|---|
| `_meta.page_path` | 全ノード | 表示先ページの相対パス。file_per 設定 ([paging-spec.md](paging-spec.md)) を参照し、自身が分割対象なら自ページ、でなければ親側を遡って解決 |

ページパスの導出規則:

- **非 root の分割対象ノード**: 自身の anchor_path の先頭 `/` を落として `.md` を付けたもの (例: `/erds/user-management` → `erds/user-management.md`)
- **root (`anchor_path == "/"`)**: `index.md` 固定。file_per 設定の有無に依らない。root は `_parent` を持たない＝遡りの終端なので常にページであり、かつ generic な `{anchor_path}.md` 式が root では退化する (`/` → `.md`) ため、特例として `index.md` にマップする
- **非分割対象ノード**: `_parent` を遡り、最寄りの分割対象ノード (無ければ root) の page_path を採る
...
`_meta` に持たせるのは path のみ。fragment 付き URL は、anchor 解決フィルタが必要時に anchor_path から合成する (詳細は [anchor-spec.md#リンク解決](anchor-spec.md#%E3%83%AA%E3%83%B3%E3%82%AF%E8%A7%A3%E6%B1%BA) 参照)。URL を事前計算しないのは、URL は `(source page_path, target page_path, target anchor_path)` の三項で決まるため、ノード単体には属さないため。

実装方針: B3 同様 `_parent` チェインを辿る lazy 評価。判定には `_meta.object_type_id` と paging 設定を引き合わせる。root の `index.md` 特例のみ式の外で扱う。
...
リンク解決は pre-render 段階で完結する (post-render の文字列置換は採らない)。各テンプレート render 用の TemplateEngine インスタンスに対し、resolver を closure binding で渡し、各種フィルタが共有する。source ページ側のパスは B6 が注入する `_meta.page_path` を `@pass_context` 経由で ctx から読む形で、TemplateEngine 側に専用の配管は持たせない (詳細は [anchor-spec.md#リンク解決](anchor-spec.md#リンク解決))。フィルタ・resolver は B2 の `build_anchor_map` を共有し、anchor path をキーに直接ノードを引く。
