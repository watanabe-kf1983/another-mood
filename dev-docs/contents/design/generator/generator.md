...
| `_meta.object_type_id` | 全ノード | このノードのスキーマ位置を表す catalog 形 ID。Mapping は ObjectType ID (`X.item`)、Array は要素型に `[]` を付けた catalog 形 (`X.item.yyy.item[]` ＝ `X.item.yyy.item` の配列)。root は `.item`（`data_catalog._item_type_id([])` が空 edge path に返す値＝root オブジェクトの型 ID）。子の合成は `[]` を含まない field-path 形 (`_type_path`) に対して行い、`[]` は公開 id にのみ付く (Array の子が `…item[].item` にならないため) |
...
- `_meta` は wrap 時に各ノードへ eager に確保。中身の `anchor_path` / `_type_path` が `cached_property` で初回のみ「親の同名値 + 自分の `_segment`」という単純な再帰合成で求まる (親側も同様に cached なので全体 amortized O(depth))。`object_type_id` は `_type_path` に Array のとき `.item[]` を足すだけ
...
- `_type_path` は `.` で join。Mapping element of Array だけ schema 上のスロットが定数 `item` になるので segment を上書きする (それ以外は `_segment` をそのまま使う)。root は `.item`。ただしトップレベル entity は catalog root (`parent_entity=None`) なので、root の id は子の prefix に**しない** — 合成上 root は空 prefix を寄与する。公開 `object_type_id` は Array ノードに限り `_type_path + ".item[]"` (要素型の配列を表す catalog 形)

### anchor_path → ノードマップ

リンク解決 (B4 / B5) は anchor path からノードを引く。`build_node_map` が `wrap_tree` の戻りを `iter_nodes` で 1 度舐め、`{_meta.anchor_path: node}` の**フラットなマップ**を構築する。root は `["/"]` で取れる ([anchor-spec.md](anchor-spec.md#id-体系) の root = `/` 不変条件に依拠)。
...
`page_path` は [`ReportsConfig`](paging-spec.md) のインスタンスメソッドに置く — 「どの object type が分割境界か (file_per)」「root→`index.md` / 分割対象→`{anchor_path}.md`」という paging ポリシーを知るのは config だから。構造的な木の遡上だけは data_tree 側の free function `nearest_ancestor(node, match)` (`self` を含め `_parent` を遡り `match` を最初に満たすノード、無ければ `None`) に切り出し、config は paging を知らないツリーに触れず、data_tree は paging を知らないまま保つ:
...
`node()` で **ノード** を得て、整形フィルタで Markdown リンク / display 文字列 / URL を生成する:

- `node(seg, *segs)` — segments（各 escape）か `/` 始まりの出来合いアンカーパスから、ノードを得る（関数・フィルタ両用）
- `link` / `label` / `href` — ノード → `[<display>](<URL>)` / display 文字列 / URL（`link(text)` で override）
...
> **未実装** — Phase 11 タスク [A6](../../../tasks.md)。prose section を最小見出し=H1 に正規化する A4（[markdown-parser-spec.md](../normalizer/markdown-parser-spec.md#見出しレベルの正規化)）の **生成時カウンターパート**。

埋め込んだ Markdown の見出しレベルを、囲うページ／セクションの深さに合わせて下げるフィルタ。stock Jinja の `{% filter %}`（任意の埋め込み出力を囲む）／パイプ（prose body）両用で、特定のタグには依存しない:
...
{{ this.body | under_heading("##") }}
```

引数は **囲う側の見出しレベル**のマーカーで、中身の全見出しを `#` の数ぶん下げる（`##` なら +2）。マーカーが結果レベルでも下げ幅の数でもなく「呼び出し側で著者が目にしている文脈の見出し」と一致するのが要点 — `## Results` の下に書くなら `under_heading("##")` と見たまま書ける。`#` 起点で書かれた中身なら、これでちょうどその見出しの直下に収まる。

#### 合成は加法的、深さの伝播は不要

各埋め込み境界で「直下の完成出力」をシフトすれば、シフトは加法的に合成される。親が子を、子が孫をそれぞれローカルにシフトすれば、孫の見出しは各段の和に着地する。どの境界も自分の直下の1段しか宣言せず、累積深さを知る必要がない。ネストした `{% filter %}` がそのまま入れ子の合成になる。

#### AST 必須

シフトは生成 Markdown を markdown-it-py（[A1](../../../tasks.md) で導入）の AST で解析して行う。素の `^#+` 置換はコードフェンス内の `#`（`query.md` は ` ```mermaid ` を吐く）、setext 見出し（`#` を持たない）、H6 上限（`#######` は見出しでない）、blockquote/リスト内見出しを壊す。自然な著述形（リテラル `#`）を許す以上、この解析は選べるトレードではなく**本質的なコスト**。

**シフト対象は ATX 見出しのみ**。`heading_open` トークンの `markup` が `#` 列なら ATX（`map[0]` の行を書き換え、シフト後レベルは H6 で clamp）、`=` / `-` なら setext として**素通し**する。コードフェンス内の `#` は heading トークンにならないので同様に素通し。setext を触らないのは暫定の限界ではなく**定義済みの挙動** — 主入力のテンプレート出力は構造的に ATX で setext は原理的に出ず、prose body の setext も実質皆無だから。ATX と setext が混在する病的ケースでのみレベル不整合が起こりうるが、必要なら検出時に warning を出せば足りる。

#### split との合成

`{% mood_view %}` 主題が split されると mood_view は `""` を返す（[paging-spec.md 分割ルール](paging-spec.md#分割ルール)）ので、`{% filter under_heading %}` は空をシフト＝無風。独立ページは別レンダリングで著者の `#` 起点のまま出る。同じ記述が split でも inline でも正しく、「同じテンプレートが Web 用(分割)でも PDF 用(全インライン)でも動く」性質を保つ。
