...
- **`iter_nodes` は free function**。`_` プレフィックスはノードのテンプレート公開フィールド専用 ([data_tree.py](../../../../src/another_mood/components/generator/data_tree.py) 冒頭参照) なので、内部専用の走査をノードのメソッドにすると命名規約と衝突する。`build_anchor_map` の走査 1 回はツリー全体を舐める他処理 (B6 の `page_url` 事前計算等) の共通基盤にも転用できる
