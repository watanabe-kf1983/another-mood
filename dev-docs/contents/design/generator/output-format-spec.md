...
`globals` / `filters` は config 非依存の静的ヘルパだが、source ページ相対の anchor リンクフィルタ (`href` / `link`、[anchor-spec.md#リンク解決](anchor-spec.md#リンク解決)) は paging 設定 (`ReportsConfig`) に依存する。これらは `OutputFormat.link_filters` を「`ReportsConfig` を受けてフィルタ群を返す factory」フィールドとして持たせ、`make_environment(output_format, reports_config)` がビルドの config で呼んで登録する。こうしてフォーマットは自分のフィルタ面全体（静的 + config 依存）を一箇所で所有し、呼び出し側が個別に配線せずに済む。
