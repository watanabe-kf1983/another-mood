...
> **未実装** — Phase 11 タスク [B1〜B6](../../../tasks.md)（ラッパーツリー / アンカー生成規則 / オンデマンド走査 / `link_md` フィルタ / `toc:id` 解決 / `get_page_url`）

### ID 体系

アンカーは **データツリー上のパス** を `/` 区切りで表現した文字列。
...
リスト要素に `id` フィールドが無い場合（Array pattern で id を schema 上要求していない場合、[schema-spec.md](../normalizer/schema-spec.md) 参照）、その要素はアンカー不可。到達経路を表現する手段がないため、配下のオブジェクトもアンカーを持たない。

#### Escape 規則

アンカーは URL fragment として埋め込まれる文字列。`/` を path 区切り文字として予約しているため、**id 値が `/` を含む場合は percent-encoding (`%2F`) で escape する**。

その他の URL-fragment-unsafe な文字（空白、`#` 等）も percent-encoding する。HTML5 の `id` 属性が空白を許容しないため、空白を含む id を持つレコードは技術的にアンカー不可（[未決事項](#未決事項)参照）。

#### Prose の例外

`prose` entity に限り、id 内の `/` を escape **せず** にアンカーへ素通しで埋め込む。
...
旧仕様（`{class}.item.{id}` 形式）と異なり、新仕様ではアンカーが **データツリー上の到達経路そのもの** で構成されるため、ネストしたリスト要素間で id が重複してもアンカーが衝突しない。

#### クラスとの関係

class（[schema-spec.md](../normalizer/schema-spec.md) の Entity ID および ObjectType ID）は **型レベルの識別子** で、アンカーとは直交する概念:
...
- **anchor**: データツリー上の実体パス（例: `categories/web/tasks/foo`）。リンク解決でのみ使われる

旧仕様では anchor を `{class}.{id}` と class 名込みで構築していたが、新仕様ではアンカーは実体パスのみで構成する。class はアンカー構築には登場しない。
...
アンカーから実際のリンク先 URL への解決は paging 設定に依存する（[paging-spec.md](paging-spec.md) 参照）。エンジンがアンカーを適切な相対パス + fragment に置換する。

例: `[ユーザー](../erds/user-management.md#erds/user-management/entities/user)`

prose 例外の resolver 側挙動: アンカー文字列を path 区切り文字 `/` で分割しつつ走査するが、`prose/` を先頭セグメントに見たときは残り全体を単一の id とみなして flat list を引く。例外は anchor 構築側（escape 省略）と整合する形で resolver にも 1 箇所だけ規則を入れる。

### 未決事項

- **空白を含む id の扱い**: HTML5 の `id` 属性は空白不可のため、空白を含む id はアンカー化不可。ビルド時に警告して当該 id 配下をアンカー無し扱いとする方針（[F4 / D 群と連携、未タスク化](../../../tasks.md)）
