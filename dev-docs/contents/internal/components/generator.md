...
1. **メタドキュメンテーション** — 内蔵テンプレートでスキーマ・クエリを可視化（[meta-documentation.md](../../design/app/meta-documentation.md) 参照）
...
3. 内蔵ルートテンプレートからレンダリング開始
   - {% mood_view %} が paginate を参照し、分割 or インライン判定
...
> **未実装** — Phase 8 タスク [B1〜B6](../../../tasks.md)。アンカーの仕様は [anchor-spec.md](../../design/generator/anchor-spec.md) を参照。
...
Markdown データソースの body には、Normalizer がソース内の相対リンクを `toc:` 記法に変換済みのリンクが含まれる（[markdown-parser-spec.md](../design/normalizer/markdown-parser-spec.md) 参照）。Generator はこれを上記と同じ仕組みで解決する。追加のリンク解決ロジックは不要。
...
- **カスタムタグ（Extension）**: `{% mood_view %}` の実装に使用
