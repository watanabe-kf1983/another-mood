...
1. **検証用スキーマ読み込み**: 検証に使うスキーマを読み込み、検証用辞書を構築する
...
| ステージ | 検証用スキーマ | 対象ファイル |
|---|---|---|
| contents | `schema_file`（ユーザ定義スキーマ） | `contents_dir/*.yaml` ※ |
| queries | 内蔵 QuerySchema | `queries_dir/*.yaml` |

※ `contents_dir` には YAML・Markdown のほかにバイナリファイル（PNG, JPG 等）も配置される想定。バイナリファイルの正規化における扱い（パス解決、Composer への受け渡し等）は未決定。Phase 8 タスク [H1, H2](../../../tasks.md)（仕様確定 + 実装）。
...
追加の Unique 制約（id 以外のフィールドに対する一意性）の宣言は将来対応とする。Phase 10 タスク [D8, D9](../../../tasks.md)。

## 背景: スキーマファイル自体の正規化が不適切な理由

スキーマファイル（`schema_file`）に対して汎用の辞書→配列正規化を適用し、その結果をメタドキュメンテーション（スキーマの可視化）に使う案を検討したが、不採用とした。
