...
なお、Generator がアンカー解決等のためにオブジェクトへのメタ情報注入を行うラッパーレイヤーを将来導入する際に、スキーマ情報に基づく未定義フィールドアクセスの検知（typo 検出）も同じ仕組みで実現できる可能性がある。
...
ルートオブジェクト以外の全オブジェクトノードに、親オブジェクトへの参照 `_parent` を付与する。YAML ロード時に自動的に設定される。
