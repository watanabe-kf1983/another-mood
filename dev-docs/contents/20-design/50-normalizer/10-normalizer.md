...
#### 背景: body の存在理由が既に否決されている

`body` は「値が自身の型を持つ」Typed Value 形だが、その機構は [output-format-spec.md の Proposals](../70-generator/50-output-format-spec.md) で「汎用性は高いが、素 string 主体の入力形式では運用負荷が見合わない」として見送り済み。escape は filter + output_format で処理する方針が確定しており、body が自己記述的である必要がない。

実態としても body は単位として使われていない:
...
ネストが構造的に必要になるのは **1 レコードが型付きの中身を 2 つ以上持つ場合のみ**。その予定はない。仮に将来フィールド単位の型付き値 (`description: {mime_type, content}` 等) を入れるとしても、それはフィールド側の話で、レコード直下の mime_type とは直交する。
