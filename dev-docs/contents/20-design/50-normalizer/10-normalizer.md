...
### prose レコードの body 廃止 (M9)

`prose` の `body: {mime_type, content}` を廃し、`mime_type` / `content` をレコード直下へ移す。

#### 背景: 型は payload ではなく envelope に置く

確立された形式 (HTTP / MIME / HTML) はいずれも型情報を payload の外、他のメタデータと同列のヘッダに置く。型が値の内側に同梱されるのは data URI のような「値が単独で旅をする」形式だけだが、このシステムで旅をする単位はレコードであり (カタログ・クエリ・FK・テンプレートすべてレコード単位)、content が単独で持ち回られる場面はない。prose レコードは MIME メッセージと同型に読める — `id` / `title` / `order_key` / `depth` / `headings` / `mime_type` がヘッダ群、`content` が payload。`body: {mime_type, content}` は、この慣行に対して型を最も内側に置いた形だった。

実態としても body を単位として使う消費者は存在しない:

- 全テンプレートが `{{ body.content | relink }}` と素通りして `.content` を取る。body を丸ごと扱う箇所はゼロ
- `body.mime_type` を汎用に読むコードもゼロ (prose.py の prose 専用パターンマッチのみ)
- `load_prose` の呼び出し元は 1 箇所で `mime_type="text/markdown"` を直書き。全レコードで定数であり、prose.py の分岐は常に真になる門番
- 導出フィールド (`title` / `order_key` / `depth` / `headings`) は既にレコード直下にフラット。ネストしているのは mime_type と content だけ

フラット化後の payload フィールド名は `content` とする (`body` ではない)。`body` は直列化形式の位置 (セクション) を指す語であり、順序を持たないレコードには指すものがない。役割語彙では envelope = レコード自体、metadata = ヘッダ群フィールド、payload の表現データに与える名が `content` (HTTP が RFC 9110 で "payload body" を "content" に改名したのと同じ整理)。

#### 背景: mime_type は Prose 型に的を絞った多態への布石

body 形の元の根拠だった Typed Value 機構 (値がスキーマに頼らず自身の型を運び、テンプレートが値を見て振る舞いを変える汎用の発想) は、一般機構としては採らない — ユーザデータに厳密な型を宣言させることがこのツールのアイデンティティであり、スキーマ非依存の値検査はそれと矛盾する ([output-format-spec.md の Proposals](../70-generator/50-output-format-spec.md) に同判断を記載)。多態を要求しうるのは組み込みコレクション (prose / blob) に限られる。将来の多態機構 (text/html の prose をページとして解釈する等) は、レコード直下の `mime_type` を分岐キーに Prose 型へ的を絞って作る。フラット化されたレコード直下の `mime_type` はその布石である。

#### 背景: 形の対称性を根拠にしない

「prose との構造対称性」を根拠に blob へ空の `body` を持たせたのが H1 の誤りだった (`content` が無い body は Typed Value ではなく、ラベルだけの空箱)。役割が根本的に違うと決めて別コレクションにした以上、形の対称性を要求する推論自体を採らない。各レコードの形は「そのレコードが実際に何を持つか」から決める。フラット化後に prose と blob が近い形になるのは帰結であって根拠ではない。

なお blob が導出フィールド (画像の `title` 等、メタデータ抽出) を持つ余地は理論上ある。その場合も prose の `title` と同じくレコード直下に並ぶだけで、body は要らない。

#### 影響と時期

**公開スキーマの破壊的変更**。`{{ body.content }}` → `{{ content }}` で、リポジトリ内は 3 テンプレート (showcase/music, showcase/starter, dev-docs) だが、利用者プロジェクトのテンプレートが壊れる。**H4–H7 完了直後、利用者が blob 参照テンプレートを書き始める前に実施する**。

対象: `content-schema.yaml` の prose 定義、`load_prose`、`prose.py` の body パターンマッチ、上記テンプレート、`docs/reference/`。
