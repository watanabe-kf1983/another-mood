...
> **未実装** — Phase 12 タスク [A7](node:/tasks/A/tasks/A7)。基本変換（A5）は実装済み（[Internal Design のリンク正規化](#リンク正規化)）。前提だった見出しノードの住所付けとリンク解決（[見出し抽出](#見出し抽出)・[anchor-spec.md の Prose の例外](../70-generator/20-anchor-spec.md#prose-の例外)、A3/A4）は実装済みで、残るは A5 が `node:` 化の際に落とす `#fragment` の透過だけ。
