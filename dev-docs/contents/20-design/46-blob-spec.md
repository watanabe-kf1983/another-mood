...
- 増分判定: stages.py が normalize へ**前回出力の実パス** (管理対象外の素の kwarg) を渡す。contents 側と size + mtime_ns が一致すれば前回出力から hardlink、不一致・読取失敗は安全側に倒して contents から実コピー (rsync の quick check と同じトレードオフ)
