...
**コピー戦略は hardlink + 増分再利用** — 運搬コストは構造的最小 (境界の実コピー 1 + hardlink) に抑える:
...
- **workspace 内 hop は全ファイル hardlink** — blob 限定でなく `transfer.link_or_copy` (dst あれば unlink → `os.link` → 別 FS・非対応 FS で `copy2` フォールバック) を全コピー地点に適用。blob 判定述語をツリーの根ごとに持つと誤判定が即 inode 共有事故になるため、「**workspace 内の全ファイルは write-once (置換は unlink→再作成)**」という一枚岩の不変条件へ単純化した。不変条件は `transfer.py` / `dir_lock.py` の module docstring に集約。`os.link` 成立条件として stage temp を出力と同一 FS に置く (dir_lock の `mkdtemp`)。
