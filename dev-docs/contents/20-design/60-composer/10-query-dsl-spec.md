...
E14 の着手時に露見し、まだ決めていない点:

- **`flatten.of` がドットで `as` が別位置のとき、空になった親を畳むか**。`of: hobby.pets, as: x` で配列を除去した結果 `hobby` が空になったとき、`hobby: {}` を残すか落とすか。「nullable はキー省略」の原則からは落とす側だが、明示していない
- **`flatten` / `join` の「既存属性との衝突」も同じ挿入に載せるか**。現状この二つは完全一致でしか衝突を見ていない（`flatten alias '...' collides with an existing attribute`）。既存の `hobby` に対する `flatten.as: hobby.x` のような接頭辞関係も弾くなら、既存の children ごと木に挿す形になる。「一つの節の出力に現れる書き込み先パス」の読みとしてはそうなるが、明示していない
