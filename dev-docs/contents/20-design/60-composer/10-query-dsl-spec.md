...
**書き込みが持つ存在情報は「行に必ず存在するか」の一つだけ**で、それが着地エッジの `required` そのものになる。そこから holder の `required` を導く（全行に在る値は holder を required にし、一部の行にしか無かった holder に書き足せばその既存の子は任意に落ちる）のはカタログ側 `Node.graft` の仕事なので、各節は自分の値を計算して渡すだけでよい:
