...
**弾くのは出力キーの完全一致だけ。** この段階ではドットはまだリテラルなので、`as: a` と `as: a.b` は別のキーであって重ならない。セグメント単位の接頭辞関係まで広げるのは E14。

**検査は一つの不変条件にまとめる**: *各節の `derive` が返すノードに、同名の子エッジが二つ無い*。現状は `flatten` と `join` が「別名 対 既存属性」を個別に見ていて、`select` の重複 `as` と `grouped` の `as` == `by` は誰も見ていない。出力 children から最初の重複名を返すだけの共通ヘルパに寄せ、メッセージと offender は節ごとに出し分ける。

- `flatten` — 既存の「`as` が `of` と異なり、かつ同名の属性がある」チェックを置き換える。`as` == `of` は `of` のエッジを置き換えている以上、出力に重複が出ないので自然に通る
- `join` — 既存の「`as` が左辺の既存属性と同名」チェックを置き換える
- `grouped` — 新規（`as` == `by`）
- `select` — 新規（重複 `as`）

offender をヘルパ側に持たせないのは、`grouped` の `by` 側エッジがカタログ由来のエッジ名（YAML の位置情報を持たない）で、エッジ名から offender を拾うと当てにならないため。
