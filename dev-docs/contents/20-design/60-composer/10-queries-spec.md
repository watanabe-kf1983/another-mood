...
クエリ名と正規化済みデータ名（テーブル名）の重複、およびクエリ名同士のファイルを跨いだ重複を禁止する（エラー）。テーブルとクエリは同一のソース名前空間を共有するため、同名はソース参照のシャドーイング＝曖昧参照になる。加工が必要な場合はクエリに別名を付ける。クエリ間参照 (E12) はソース名の一意性を前提とするため、E6 はその前提タスクでもある。

> 現状は Composer が正規化済みデータと同名のクエリを silent に上書きする。Phase 13 タスク [E6](node:/tasks/E/tasks/E6)。
...
- **パイプライン固定順序の逃し弁の実体化**: 本 DSL は句の順序を固定しており、順序に収まらない形（途中段 flatten 等）への公式の答えは「別 query に分割する」（「パイプライン順序」節）。しかし現状は分割した後段が前段を参照できないため、実際の回避策は共通前段の複製かテンプレート側での再結合になってしまう
...
#### 背景: インラインサブクエリを採らない理由

`from:` にクエリオブジェクトをネストさせる案（SQL のサブクエリ相当）は採らない:
...
- **依存抽出**: 各クエリの依存 = `from:` のソース名 + 各 `join[].to:` のソース名のうち、クエリ名を指すもの。builtin クエリ (`__builtin`) も同一名前空間で対象
- **処理順**: 手書き Kahn 法（残り依存数カウント + キュー）。`graphlib.TopologicalSorter` はサイクル検出時に全体を abort するため不採用 — サイクル圏外のクエリは部分進行させ、診断を出し切りたい
- **derive 側** (query_deriver): 現状のファイル単位ループを「全ファイルのクエリを収集 → topo 順に derive し、導出した view entity を catalog へ逐次フィードバック → 出力時にファイル単位へ戻す」に再構成
- **apply 側** (composer): 評価ループを同じ topo 順に。評価結果を `sources` に書き戻す構造は既存のまま
- **提示順は不変**: `__definition.queries` の並びはファイル順のまま。topo 順は評価にのみ使う

#### 診断

- **サイクル**: Kahn の残り物集合（in-degree が 0 にならなかったクエリ）がサイクル圏。残り物の各ノードは必ず残り物内に依存先を持つので、そこを辿れば閉路が 1 本抽出できる。1 閉路 1 診断とし、メッセージに閉路全体（`a → b → a`）を載せ、アンカーは参加者いずれかの `from:` の YAML 位置に置く
