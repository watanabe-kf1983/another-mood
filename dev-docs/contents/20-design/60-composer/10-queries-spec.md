...
- **処理順**: `graphlib.TopologicalSorter` で topo ソート。acyclic なら全順序が得られ、derive 失敗のカスケード抑制（後述）はその順序上で行える。サイクルがあると `CycleError` で全体を abort し部分順序は返らない。自前 Kahn 法で「サイクル圏外も部分進行させ、全サイクルを 1 度に報告する」案も検討したが、閉路抽出コードが重い割にサイクルは通常単一の構造ミスで、ビルドも高速反復のため、初回サイクルのみ報告して中断する方を採った
