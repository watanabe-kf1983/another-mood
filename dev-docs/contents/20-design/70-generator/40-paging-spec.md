...
meta 診断ページ（`__entity_defs` / `__entity_data` / `__views`）の主題は **実データツリーノード**（`resources/views/` のビルトインクエリが生む）で、分割は通常の[分割ルール](#分割ルール)そのもの。meta には利用者の `reports.yaml` が無いので paging は固定（`META_EDITION.paging`）。
