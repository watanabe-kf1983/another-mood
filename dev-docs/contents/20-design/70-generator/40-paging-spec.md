...
> **残: `__db/` への移動（F9, 前提 C12）.** これら `__{view}/` は今も output 直下に横並びで散る。集約は meta-edition の `name` を `__db` に倒して `{out_dir}/__db/__{view}/` へマウントする形で行う（[Proposals](#proposals) 参照）。
...
診断サーフェスの再配置とトップページの再設計。現状 generator はメタ描画（`out_dir` 直下に index ＋ `__entity_defs/` 等）を特別扱いし、ユーザ edition を別ループで回す。これを二段階で組み替える — 先に内部を一般化（振る舞い不変）し、次に利用者向けの移動＋表紙を乗せる。既存の editions (form B) の設定・制約は [External Design](#レポート設定ファイル)、メタ index の列挙は [Internal Design](#メタ-index-の-edition-列挙) を正本とする。`Edition` ドメイン型・`load_editions` の form A/B 分岐・`dir_segment`・`generate()` の逐次ループは `edition.py` / `generator.py` の docstring を参照。

### render ループの一般化 (C12)

前提リファクタ（振る舞い不変）。メタ描画とユーザ edition の 2 経路を、`templates_dir` / `root_template` / `file_per` / `dir_segment` / 追加 filters を差分フィールドに持つ **heterogeneous な edition kind 群**を回す単一ループに畳む。`generate()` は今 `META_EDITION` を root マウント（`name=""`）で先に render し、続けてユーザ edition ループを回すが、これを `[meta_edition, *user_editions]` の一様な反復にする。ここで per-edition の `root_template` / `templates_dir` 化が入る（`generate()` 奥の `"index.md"` ハードコードと `templates_dir` 引数を `Edition` 由来に移す）。これは下の「Edition 別ルートテンプレート（将来）」が想定していた継ぎ目そのもの — 機構は C12 で入り、利用者への露出だけが将来に残る。

- meta-edition は **root マウント（`name=""`）のまま据え置く**ので、出力はバイト単位で不変。振る舞い不変リファクタとして出力 diff が空であることで検証できる。集約・表紙は F9 に委ね、このタスクは純粋な構造変更に留める。
- **「ループが回す edition 群」と「トップが宣伝する `__editions`」は別チャネル**に保つ。meta-edition は render-list に居るが `__editions`（[メタ index の edition 列挙](#メタ-index-の-edition-列挙) が列挙する対象）には入れない — これが「内部は edition の一種、利用者向けにはそう見せない」を構造として担保する継ぎ目。
...
- **root に二セクションの薄い表紙テンプレ**を新設する: `## User Reports`（各 edition を大々的に列挙）＋ `## Database Information`（`__db/` へ控えめにリンク）。C12 で足した「root ページを持つ cover edition（`name=""`・専用テンプレ・`file_per` なし）」を `[cover, meta(__db), *user]` の先頭に据える形で乗る。edition 列挙（現 `## Reports`）は meta index からこの表紙へ移す。
...
edition ごとに異なるルートテンプレート（`index.md` 以外）を**利用者が `reports.yaml` で指定したい**需要（Web 版と印刷版でトップ構成を変える等）。**機構（`Edition` の `root_template` / `templates_dir` フィールドと `generate()` ループでの差し替え）は C12 で入る**ので、残るは reports.yaml への設定露出のみ。加算的拡張で、C12 / F9 のスコープからは外す。
