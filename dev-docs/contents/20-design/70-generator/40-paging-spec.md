...
book edition（全 inline）で prose のフォルダ親子をネストさせる仕組みは、[分割ルール](#分割ルール) の two-loop（`| link` + `{% mood_view %}`）と `under_heading` を、フォルダ木へ一般化したもの。各 prose レコードは id 由来の `order_key`（folder-preorder ソートキー）と `depth`（見出しレベル）を持ち、ルートテンプレ（`definition/templates/index.md`）が `order_key` でソートし `depth` に応じて `under_heading` で包む。両フィールドは id のみの純導出なので prose の preprocess（`prose.py` の `_outline_position` が正本）で供給し、ソートはファイル単位で行えないため collection の揃うテンプレ側に置く。
...
1. 番号規約の確定（桁・隙間の刻み・ディレクトリにも番号・`index` は無番号）
2. リネーム + 参照貼り直し: ファイル / ディレクトリに番号付与（エディタの rename 追従で相対 `.md` リンクは自動更新、`tasks.yaml` の `spec:` / `proposal:` は手動）。フォルダ単位でコミット
3. 順序検証: 再ビルド、未解決 `node:` grep、両 edition で読む順を確認、テンプレ無改修で番号が効くことを確認、`make ci` → PR

完了時は本 Proposal を削除する（ネスト機構は Internal Design 済み）。
