...
`{% mood_view "template.md" with data %}` でサブテンプレートを呼ぶ場合、`template_name` の拡張子から output_format を引き、対応する Environment で render する想定。テンプレート参照に拡張子が含まれていること（[P2](node:/tasks/P/tasks/P2) で確立、利用者向け仕様は `docs/reference/template.md` の mood_view）に依存する。
