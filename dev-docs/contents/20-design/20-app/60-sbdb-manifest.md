...
tools:                      # 任意 — 各プロセッサの名前空間
  another-mood:
    minimum_version: "0.3.5"   # 道具2 — このツールの最低バージョン
```
...
#### 背景: `minimum_version` が素のバージョンである理由

レンジ記法（`>=0.3,<0.5` / `!=0.4.2` 等）は採らない。判断基準は
「**マニフェストはソースの性質の宣言であり、書く時点でソースについて真である事実しか
書けない**」:

- 「このソースはバージョン X で入った機能に依存する」はソースの性質。書く時点で確定し、
  ソースを変えない限り変わらない。最低バージョン宣言はこれの直接の表現
- 上限（`<0.5`）は書く時点で書き手が知り得ない — 何が壊れるかは将来のリリースが決める。
  前方非互換の判定はツール側の対応集合（`sbdb_version` ゲート）にしか置けない
- 除外（`!=0.4.2`）は「ツールの性質」（特定リリースのバグ）であり、ソースは何も
  宣言していない。対処の所有者はツールのリリースチャンネル（yank・修正版）か環境側の
  インストール指定。マニフェストはビルド拒否しかできず、正しい版を選ぶことはできない

先例もこの線で割れている: 依存解決器のレンジ語彙を持つエコシステム
（npm `engines` / pip `requires-python` / Terraform `required_version`）はその語彙を
借りてレンジを受け、「ビルドツールがソースのマニフェストを読む」文脈
（Go `go 1.21` / Rust `rust-version` / CMake `cmake_minimum_required` /
pre-commit `minimum_pre_commit_version`）は一貫して素のバージョンを取る。
本ツールは後者で、レンジ語彙をどこにも持たない。

素のバージョンには派生的な利点もある: 演算子つき入力は「バージョンとして読めない」
という自明なエラーになり、「上限は書けません」のような説明を要する専用の禁止規則が
不要。将来レンジが本当に必要になれば別欄の追加は additive にできる。
...
厳格検証 → `title` 抽出 の順に進む。対応集合は `SUPPORTED_SBDB_VERSIONS`（現在 `{1}`）。

- **`minimum_version` ゲートが最初** — 「mood を X 以上に上げてください」は利用者が
  取れる最も正確な行動なので、世代ゲートにも未知キー検出にも先行させる
- **バージョン比較は `packaging`**（PyPA の PEP 440 参照実装）— mood 自身の版が
  PEP 440 なので、プレリリース（`0.4.0.dev1` など）を含む順序規則をそのまま借りる。
  `Version` 同士の `>=` 比較のみで、`SpecifierSet` は使わない（素のバージョンしか
  受けないため）
- **パース不能な `minimum_version`（演算子つき等）は `ManifestError`** — `UserStr` の
  位置情報で行番号つき診断を出す。文字列でない値はゲートを素通りさせ、厳格検証に委ねる
  （`sbdb_version` の ill-typed と同じ扱い）
- **不足時は `MinimumVersionError`** — `UnsupportedSbdbVersionError` と同格の専用例外
  （診断なし・メッセージのみ）
- **`tools.another-mood` 配下は厳格検証** — mood の語彙なので未知キー（typo した
  `minimum_version` 等）は道具3 の対象。他プロセッサの名前空間は素通しのまま
...
読み込み・厳格検証・`title`（[V1](node:/tasks/V/tasks/V1)）、`sbdb_version` の値ゲート
（[V4](node:/tasks/V/tasks/V4)）、`minimum_version` の fail-fast
（[V2](node:/tasks/V/tasks/V2)）は実装済み。残りはカテゴリ V の 1 件:

- [V3](node:/tasks/V/tasks/V3) — 生態系への展開: `mood init` / blueprint の生成、
