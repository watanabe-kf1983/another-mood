...
生成時に実行中バージョンを機械的にスタンプしてよい（Internal Design の scaffold 節）。
...
- **template** — 完備。未知タグ = `TemplateSyntaxError`、未知フィルタ =
  ...
  そこはユーザ定義領域）
- **layout** — 二層に分かれる。プロジェクト直下は `.git` / `README.md` 等との同居が
  設計であり loud 化は構造的に不可（scaffold の `cargo init` 型と同根の判断）。
  mood の管轄領域である `definition/` 配下は拒否できるのに現状 silent —
  [V5](node:/tasks/V/tasks/V5) で塞ぐ
...
### scaffold の生成（`components/scaffold`）

`mood init` / `mood blueprint apply` はコピー先に `sbdb.yaml` を新規生成する
（`sbdb_version: 1` + `title: <コピー先ディレクトリ名>` +
`tools.another-mood.minimum_version: <実行中バージョン>`）。ブループリント自身の
`sbdb.yaml` はコピーせず、書き込み対象との衝突は何も書かずに全体を事前エラーにする
（`cargo init` / `uv init` 型）。個々の挙動の理由と代替案（ファイル単位 skip・
blueprint manifest の複製）の却下理由は `blueprints.py` の docstring と
コミットメッセージに記載してある。

衝突判定は書き込み対象のファイル単位から、管轄領域の根（`sbdb.yaml` / `definition/` /
`contents/`）の存在チェックへ置き換える（[V6](node:/tasks/V/tasks/V6)）。ファイル単位
だと既存 `contents/` への blueprint のマージ（半混在状態）がすり抜けるため。cargo init
が既存 `src/` を採用するのとは違い、blueprint はサンプルデータなので採用の意味論は
成立せず、拒否が正解。

showcase / dev-docs の実物マニフェストも `minimum_version` を最新バージョンに固定して
おく（scaffold の生成物と同じ形の実例を兼ねる）。バージョン bump 時の更新はリリース
チェックリスト（[Q6](node:/tasks/Q/tasks/Q6)）の項目とする。
