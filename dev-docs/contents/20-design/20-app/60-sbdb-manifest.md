...
1. [V1](node:/tasks/V/tasks/V1) — `sbdb.yaml` の読み込みとスキーマ検証（未知キー拒否・
   ...
   フォールバックに降格）
2. [V4](node:/tasks/V/tasks/V4) — `sbdb_version` の値ゲート（対応集合照合。二段読みの
   ゲート段を厳格検証の前に挿入）
3. [V2](node:/tasks/V/tasks/V2) — `tools.<processor>.requires` の fail-fast
4. [V3](node:/tasks/V/tasks/V3) — 生態系への展開: `mood init` / blueprint の生成、
