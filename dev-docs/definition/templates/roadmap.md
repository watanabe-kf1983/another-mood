...
{% for row in group.tasks %}
    {{- "" }}| {{ node("tasks", row.id, "tasks", row.task.id) | link(row.task.id) }}
    {{- "" }} | {{ row.task.title }}
    {{- "" }} | {% if row.task.proposal %}{{ node(path="/prose/" ~ row.task.proposal.split("#")[0]) | link("→") }}{% endif %}
    {{- "" }} | {{ row.task.note | replace('\n', ' ') | trim }}
    {{- "" }} | {{ "✅" if row.task.done else "" }}
    {{- "" }} |
{% endfor %}
