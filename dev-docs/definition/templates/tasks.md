...
残タスクを機能カテゴリ別に整理したもの。各タスクは実装箇所と仕様ドキュメントを併記する。フェーズ別の順序は {{ node(path="/roadmap") | link("ロードマップ") }} を参照。完了済みタスクは記載しない（git log を参照）。
...
{% if category.spec %}
**仕様:** {{ node(path="/prose/" ~ category.spec) | link }}
{% endif %}
