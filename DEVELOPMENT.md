...
タスク開始時には `mood build docs-src` でドキュメントをビルドし、仕様が参照できる状態にする。
...
設計ドキュメントの変更は、必ず `docs-src/contents/` 配下のソースに対して行い、変更後は `mood build docs-src` を実行して `.another-mood/docs-src/output/` の出力を確認する。
...
- [index.md](.another-mood/docs-src/output/prose/index.md) — 全体インデックス
- [external/index.md](.another-mood/docs-src/output/prose/external/index.md) — 外部仕様インデックス
...
ドキュメントの閲覧先は `.another-mood/docs-src/output/`（ビルド方法は [docs/README.md](docs/README.md) を参照）。

- [index.md](.another-mood/docs-src/output/prose/index.md) — 仕様・設計
- [background/product.md](.another-mood/docs-src/output/prose/background/product.md) — 製品ビジョン
- [internal/architecture.md](.another-mood/docs-src/output/prose/internal/architecture.md) — アーキテクチャ
- [background/roadmap.md](.another-mood/docs-src/output/prose/background/roadmap.md) — ロードマップ
