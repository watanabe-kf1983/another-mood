...
- **`docs/`** — 利用者向け文書。[Diátaxis](https://diataxis.fr/) に従い、`reference/` は reference、`guides.md` は tutorial / how-to / explanation として書く。素 Markdown、英語。実装済み機能のみ。人間の可読性と、読んだ LLM の振る舞いの双方が UX に直結するため、品質に妥協しない。書かれたことは外部仕様の正本（利用者との契約）としても扱われるが、それは帰結であって執筆の目的ではない
...
- **`dev-docs/dev/`** — 開発規約・手順の正本（セットアップ / 環境 / チェック / スタイル / リリース）。日本語。簡潔・規範的に書く

`Proposals` の検討メモは、実装完了時に削除する（部分実装なら残る検討事項のみに絞る）。維持価値のある設計判断は External / Internal Design 節に移すか、code docstring や PR 本文で残す。

設計判断の背景・理由は、判断が書かれている場所に直接書く（ADR のように別ファイルに分離しない）。`design/` 内では「## 背景: ...」セクションとして該当箇所の近傍に書き、実装済み機能ではコードの docstring か PR 本文で残す。
...
1. 以下をタスクに応じた順序で進める:
    ...
    - **実装**
2. **showcase/ で動作確認**
3. **`docs/` を同期** — 利用者が書ける形・得られる形（CLI / スキーマ / クエリ / テンプレート、内蔵リソース等）が変わったら `docs/` の該当箇所を更新する
...
PR 本文はそのまま main の squash コミット本文になる（ブランチ上のコミットメッセージは残らない）。変更の要約と、設計判断の背景・経緯はここに書く。タスク ID もここ——タイトルは利用者が読むリリースノートに載るので、内部語彙を置かない。
