...
is an edge name with ``!`` for a required edge and ``[]`` for a
collection, so ``a!.b`` is an object ``a`` on every row holding a ``b``
that some rows lack, and ``arr[]!.x!`` is a required array of objects
