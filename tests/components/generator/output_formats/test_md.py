...
class TestMdAnchor:
    def test_returns_markup(self) -> None:
        assert isinstance(
            md_anchor(build_node_map(ensure_inert(_ANCHOR_DATA))["/members/alice"]),
            Markup,
        )

    def test_emits_closed_anchor_with_node_anchor_path(self) -> None:
        node = build_node_map(ensure_inert(_ANCHOR_DATA))["/members/alice"]
    ...
    def test_prose_heading_emits_nothing(self) -> None:
        ...
        heading = build_node_map(ensure_inert(_ANCHOR_DATA))[
...
class TestMdPostProcess:
    ...
    def test_stamps_root_node_anchor(self) -> None:
        root = build_node_map(ensure_inert(_ANCHOR_DATA))["/"]
...
def _anchors() -> dict[str, Node]:
    return dict(build_node_map(ensure_inert(_ANCHOR_DATA)))


def _paging() -> PagingPolicy:
    return PagingPolicy(_ANCHOR_FILE_PER)
...
class TestMakeLinkFilters:
    ...
    def test_returns_href_link_anchor_and_relink(self) -> None:
        filters_map = make_link_filters(_paging(), _anchors())
...
class TestLinkFilterWiring:
    ...
    def _engine(self, tmp_path: Path, body: str) -> TemplateEngine:
        ...
        return TemplateEngine(
            ...
            output_format=MD,
            filters={**node_filters, **make_link_filters(_paging(), _anchors())},
            globals=globals_map,
            paging=_paging(),
        )
