...
パイプラインをワンショット同期実行し、結果を返す。CLI の `reqs build` と同一のパイプライン（Hugo 描画含む）を実行する。

AI エージェントのフィードバックループ向け: edit → build → 結果取得 → 判断 → edit → build → ...

### dev start / dev stop

dev server（ファイル監視 + パイプライン自動再実行 + Hugo プレビューサーバ）をバックグラウンドで起動・停止する。人間がブラウザでドキュメントを閲覧するためのもの。エージェントが人間の指示に応じて制御する。
...
将来的には example-project やメタスキーマ（query-schema.yaml, schema-schema.yaml）も output に含める。メタスキーマには JSON Schema の title / description フィールドで説明的なメタデータを埋め込み、スキーマ自体を自己説明的にする。
...
ドキュメントの原本は docs-src に一元管理し、reqs-builder 自身でビルドした output を複数チャネルで提供する:
...
## 背景: build と dev の同時実行

build（エージェントのワンショット実行）と dev（バックグラウンドのファイル監視）は同時に動作しうる。エージェントがファイルを編集すると dev の watch が検知してパイプラインを起動し、その後エージェントが build を呼ぶケースがある。
...
パフォーマンス上の二重実行コストが問題になった場合は、dev の watch を一時停止する仕組み（pause_watching / resume_watching）の導入を検討する。

## 背景: dev モードが AI エージェント向けに不要な理由

AI エージェントのツール実行モデルは同期的なリクエスト→レスポンスである。常駐プロセスのログストリームから特定の変更に対する結果を抽出するのは困難であり、ワンショットの build で結果を同期取得する方がフィードバックループに適している。

ただし dev server はエージェントの背後にいる人間のために必要である。人間はブラウザでリアルタイムにドキュメントを確認したく、その仕組みは人間の直接編集・エージェント経由の編集のいずれでも機能する必要がある。そのためエージェントが dev server の起動・停止を制御できるようにする。
