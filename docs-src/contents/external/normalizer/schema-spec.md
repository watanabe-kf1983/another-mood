...
`schemaDir`（デフォルト: `docs/definition/schema/`）配下に YAML ファイルとして配置する。スキーマファイルには `schemas` と `references` の2種類のトップレベルキーがある。1ファイルに両方を書いても、別ファイルに分けてもよい。ファイルの分割・統合・リネームはスキーマ名や参照関係に影響しない。

```yaml
# {schemaDir}/entities.yaml — ファイル名は整理用
schemas:
  users:
    type: object
