...
| プレフィックス | 用途 | 例 |
|---|---|---|
| `_`（単一） | ユーザが参照するシステム定義フィールド | `_prose`, `_mime_type`, `_content` |
| `__`（二重） | システム内部フィールド（ユーザは直接扱わない） | `__errors` |

`_` プレフィックスのフィールドはテンプレートやクエリで参照可能。`__` プレフィックスのフィールドはパイプライン基盤が使用し、ユーザのテンプレートやクエリからは参照しない。

現在 `prose` として出力している内蔵スキーマのキーは、ユーザ定義との衝突を避けるため `_prose` に変更予定。
