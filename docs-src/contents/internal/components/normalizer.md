...
- エラーモデルの整理: 現在 `errors.py` の `errors_data` はダックタイプで例外から `filename`/`lineno` を推測しているが、`errors.py` で `source` と `lineno` を持つ Exception クラスを定義し、各コンポーネントが外部ライブラリの例外をその形に変換する責務を持つ設計に変更する
