# Process Coordination

コンポーネント間のプロセス連携仕様。

## カスケード監視パターン

各コンポーネントは以下のパターンで連鎖的に起動される:

```
┌─────────────────────────────────────────────────────────┐
│ Watcher                                                  │
│   Watch 対象フォルダを監視（debounce 付き）               │
│   AtomicDirWriter を起動                                 │
└──────────────────────┬──────────────────────────────────┘
                       ↓
┌─────────────────────────────────────────────────────────┐
│ AtomicDirWriter                                          │
│   processFn を実行し、出力の原子性と順序性を保証する       │
│   詳細は atomic-dir-writer.md を参照                           │
└──────────────────────┬──────────────────────────────────┘
                       ↓ 出力フォルダ（メタファイル含む）
┌─────────────────────────────────────────────────────────┐
│ Watcher（次段）                                           │
│   前段の出力を監視 → 次段を起動                            │
└─────────────────────────────────────────────────────────┘
```

この連鎖により、上流の変更が下流へカスケードで伝播する。

Watch 対象と Input（processFn が実際に読むデータ）は必ずしも一致しない。
各ステージの Watch / Input の対応は [pipeline.md](pipeline.md) を参照。

## Watcher の監視対象

下流の Watcher は、上流の outputDir を監視対象に含める:

```
// 例: Composer が normalizedContentsDir の変更を監視する場合
watcher.on([
  '{normalizedContentsDir}/**'
], { debounce: 300 }, () => compose())
```

AtomicDirWriter はディレクトリの inode を維持したままインプレースで中身を更新するため（[atomic-dir-writer.md](atomic-dir-writer.md) 参照）、OS のファイル監視イベントで確実に検知される。

## エラーの扱い

エラーは生成ドキュメントとして出力される。各コンポーネントはエラー情報を
正常な結果と区別のない形で出力し、後続のコンポーネントに通常のカスケードで連携される。

## AtomicDirWriter

出力の原子性・順序性の仕様は [atomic-dir-writer.md](atomic-dir-writer.md) を参照。

## 設計判断

### ファイルを介した連携を選択した理由

- **デバッグ容易性**: 各段階の結果を YAML ファイルとして目視確認できる
- **疎結合**: 各コンポーネントが別言語・別プロセスでも動く
- **クリーンビルド**: `rm -rf .reqs-builder/` で全生成物を一括削除できる

### 下流が invalid 時にエラーページを生成する理由

- 壊れたリンクを含むドキュメントを出力しない
- 読者に「データが壊れている」ことを明示できる
