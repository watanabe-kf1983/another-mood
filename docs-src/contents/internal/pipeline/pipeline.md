...
理由: `exclusive_write` は Output ディレクトリの **子**を rmtree してから copytree で書き戻す設計 (`components/shared/exclusive_write.py` 参照)。このとき子ディレクトリの inode が入れ替わる。**inotify の watch は inode に紐付く**ので、watch 対象が子ディレクトリだと rmtree で watch が無効化され、以降の event を受信できなくなる。親ディレクトリを Watch すれば親の inode は保持されるため、子の rmtree/recreate を跨いで event を継続受信できる。
