...
normalized データは自動的に views にコピーされる（パススルー）。`model/data/` に配置したデータは、クエリを書かなくてもテンプレートから参照可能。クエリは追加の view（join、集計等）を定義する場合にのみ記述する。

### 同名禁止

クエリ名と normalized データ名（テーブル名）の重複は禁止する（エラー）。クエリの `from:` は常に normalized データ（テーブル）を指すため、同名を許すと循環参照が生じる。加工が必要な場合はクエリに別名を付ける。
...
views は `output/model/views/` に YAML ファイルとして書き出される。自動パススルー分と、クエリ評価結果の両方が含まれる。
...
output/model/views/
  ...
  screen-details.yaml # queries/screen-details.yaml の評価結果
```
