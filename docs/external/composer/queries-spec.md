...
クエリは `normalizedDir` の正規化済みデータに対して評価される。テンプレートは常に `viewsDir` のみを参照し、normalized データを直接使うことはない。

### 自動パススルー

normalized データは自動的に views にコピーされる（パススルー）。`contentsDir` に配置したデータは、クエリを書かなくてもテンプレートから参照可能。クエリは追加の view（join、集計等）を定義する場合にのみ記述する。
