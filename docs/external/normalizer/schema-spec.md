...
`schemaDir`（デフォルト: `docs/definition/schema/`）配下に YAML ファイルとして配置する。1ファイルに複数スキーマを定義可能で、**トップレベルキーがスキーマ名**となる（ファイル名はスキーマ名に影響しない）。

```yaml
# {schemaDir}/entities.yaml — ファイル名は整理用、スキーマ名はトップレベルキー
users:
  type: object
