...
prose:
  - id: "internal/architecture.md"
    ...
    body: |
      ...
      ## アーキテクチャ概要
      ...
    relative-path: "internal/architecture.md"
```
...
YAML frontmatter があればフィールドとしてマージする:
...
Frontmatter は任意。なければ `id`, `title`, `body`, `relative-path` のみのレコードが生成される。エコシステム（GitHub、VSCode、Hugo 等）が広くサポートしており、ソースの可搬性を損なわない。
...
```markdown
処理フローの詳細は[Composer](toc:prose.item.internal/composer.md)を参照。
