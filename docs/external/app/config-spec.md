...
| キー | 型 | デフォルト | 環境変数 | 説明 |
|------|-----|---------|----------|------|
| `schemaDir` | string | `./docs/definition/schema` | `RB_SCHEMA_DIR` | スキーマ定義ディレクトリ |
| `contentsDir` | string | `./docs/contents` | `RB_CONTENTS_DIR` | ソースデータディレクトリ（YAML + Markdown） |
| `queriesDir` | string | `./docs/definition/queries` | `RB_QUERIES_DIR` | クエリ定義ディレクトリ |
| `templatesDir` | string | `./docs/definition/templates` | `RB_TEMPLATES_DIR` | テンプレートディレクトリ |
| `profilesFile` | string | `./docs/definition/profiles.yaml` | `RB_PROFILES_FILE` | プロファイル設定ファイル（ページ分割戦略） |
...
| キー | 型 | デフォルト | 環境変数 | 説明 |
|------|-----|---------|----------|------|
| `meta.outDir` | string | `./.reqs-builder/meta/output` | `RB_META_OUT_DIR` | メタドキュメンテーション Generator の出力先 |
| `meta.render.outDir` | string | `./.reqs-builder/meta/render` | `RB_META_RENDER_OUT_DIR` | メタドキュメンテーション Renderer の出力先 |
