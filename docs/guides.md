...
通読用ガイド。Quick Start で 1 つ動くプロジェクトを動かしてから、コンテンツ → クエリ → テンプレートの順に「自分のプロジェクトに置き換えるとどう書くか」を解説する。構文の引きには [Reference](reference/index.md) を使う。

## another-mood とは

要件定義書、開発者向けの ER 図、運用手順書、サポート向け FAQ — これらのドキュメントには「ユーザ」「注文」「画面」といった同じ登場人物が、形を変えて何度も出てくる。エンティティを 1 つ追加するたびに 4 箇所を直して回り、どこかで直し漏れて、いつの間にか食い違っている — そういう類のドキュメント群を保守している人のためのビルドツールが another-mood。

書き手は、データ（YAML）と散文（Markdown）を 1 箇所に書く。出したい各ドキュメントの形は、テンプレートで宣言しておく。データを 1 行直せば、紐付くすべてのドキュメントが整合した状態で再生成される。

ただ、このやり方は「思いついたことを気軽に Markdown で書く」スタイルとは相容れない。データを書く前にスキーマを宣言する手間や、ページ構造をテンプレートで分割する手間が乗る。書き手にプログラマ的な構造化思考を要求するツールだという点は、最初に断っておく。だから、軽い読み物・blog・1 度しか読まれない文書はこのツールの対象外。同じ事実が形を変えて何度も登場するドキュメント群の、整合の維持コストを下げるためのツール。

### 必要なもの

- Python 3.x（[uv](https://docs.astral.sh/uv/) で管理）
- [Hugo](https://gohugo.io/)（HTML プレビュー用）

## Quick Start

最小サンプルを動かして、入力ファイルと出力ファイルがどう対応しているかを目で見るところまで。

### インストール

[uv](https://docs.astral.sh/uv/getting-started/installation/) と [Hugo](https://gohugo.io/installation/) を入れたうえで:
...
CLI は `uv run mood ...` で動かす。以降の例では `uv run` を省略する。
...
`mood init` で `my-project/` 配下にメンバー名簿の最小サンプルが入り、`mood build` でそれを Markdown と HTML に変換する。
...
├── definition/
│   ├── schema.yaml              # データの型
│   ├── queries/by_role.yaml     # ロール別に束ねるビュー
│   └── templates/
│       ├── index.md             # トップページ
│       ├── member.md            # メンバー詳細
│       └── by_role.md           # ロール別一覧
...
生成された側（`.another-mood/my-project/` 配下）:
...
│   └── __meta_entity, __table_view, __meta_query   # 後述
└── render/                                   # HTML（Hugo 出力）
```

メンバーが 3 人いるので `member/` 配下に 1 ファイルずつ、ロールが 2 種なので `by_role/` 配下に 1 ファイルずつ生成されている。テンプレートの `index.md` がデータをループしながらサブページを書き出す仕組みになっていて、これがこのツールの中核。詳しくはテンプレート章で。

`__` で始まるディレクトリは、ユーザのテンプレートとは独立にツールが自動生成するページ。スキーマ・データ・クエリの現状を執筆中に確認するためのもので、執筆フロー章で詳述する。
...
`http://localhost:1313` でブラウザに HTML が出る。ファイルを編集して保存すると自動で再ビルド + リロードされる。試しに `contents/members.yaml` の `alice` の `role` を `engineer` から `manager` に変えてみる。`by_role/manager.md` が新しく現れて、`by_role/engineer.md` から alice が消えるのが見える。データを 1 行直しただけで、複数のページが整合した状態で再生成された — これがこのツールがやりたいこと。

ここから先は、このサンプルを足がかりに、自分のプロジェクトを書くために必要な道具立てを順に解説する。

## 執筆フロー

サンプルを真似て自分のプロジェクトを書き始めるとき、どこに何を置き、どんな順番で作っていくか、という運用面の話。
...
| 何を書くファイル | 場所（既定） |
|---|---|
| データの型（必須） | `<project>/definition/schema.yaml` |
| 構造化データ | `<project>/contents/*.yaml` |
| 散文 | `<project>/contents/**/*.md` |
| クエリ（任意） | `<project>/definition/queries/*.yaml` |
| テンプレート（最終出力を出すなら必須） | `<project>/definition/templates/*.md` |

`schema.yaml` だけは 1 ファイル固定。それ以外は複数ファイル・サブディレクトリに自由に分割できる。パス自体の上書きは [CLI](reference/cli.md) を参照。
...
| 段階 | 確認できるページ | 何を確認するか |
|---|---|---|
| 1. スキーマを書く | `output/__meta_entity/<entity>.md` | 宣言した型がツールにどう解釈されたか |
| 2. データを書く | `output/__table_view/<entity>.md` | データが期待通り読み込まれているか |
| 3. クエリを書く | `output/__meta_query/<query>.md` | クエリの結果がどんな形か |
| 4. テンプレートを書く | `output/index.md` 以下 | 最終ユーザ向けのページ |

`mood watch` を回したまま編集すれば、各段の成果が即座にブラウザに出る。テンプレートを書き始める段階では、参照するデータ・クエリの形が既に確定しているので、テンプレートに集中できる。

### `mood build` と `mood watch` の使い分け

エディタで書きながらブラウザで結果を確認したいとき（人間が継続的に書いているとき）は、`mood watch` を起動して放置する。一方、CI で生成だけしたいときや、エージェント（Claude Code 等）が「編集 → ビルド成否を確認 → 次の編集」を回したいときは `mood build`。`watch` は常駐するので終了コードでの判定はできない、というのが使い分けの基準。

## コンテンツとスキーマ

書き手が手で書くソースは、構造化データ（YAML）と散文（Markdown）の 2 種。書き方が違うので別々に説明する。
...
`as: id` を付けているのには理由がある。テンプレート側でこのビューを使って「ロール別ページを 1 ロール 1 ファイルで出力」したい（テンプレート章で詳述）。サブページに分けるには、各エントリに `id` フィールドが必要。`id` を付けないと、出力が 1 ファイルに集約されてしまう。
...
テンプレート側はデータの出所を意識する必要がなく、執筆中に「やっぱりデータをクエリ経由に置き換える」とした場合も、参照名が変わるだけでテンプレートの書き方は同じ。
...
`templates/index.md` を起点に固定しているのは、サイト全体の入口を 1 つに定めるため。読者・別の執筆者・LLM のいずれにとっても「最初に読むファイル」を探さなくてよい。
...
代償として、スペルミスもエラーにならず黙って空になる。これは、書きながら `__table_view/` でデータの実体を見て、`__meta_query/` でクエリの結果形を確認する執筆フロー（執筆フロー章「内側から外側に書く」）でカバーする想定。
