...
**another-mood** は、ソースベース DB の投影機 (projector) で、そういうドキュメント群の整合維持を担う。ここでいう **ソースベース DB** とは、ユーザ自身が作成・更新・削除するファイル群 ── 以下「**ソース**」と呼ぶ ── からなるデータベース。ソースは YAML や Markdown 等の形式で、データ・登場人物の型定義・クエリ・ドキュメントテンプレートを含む。ユーザがエディタ等でソースを直接書き換えるのが唯一の DB 操作の手段。一方、another-mood はそれを読み込み、クエリの実行結果・データの一覧・テンプレートに基づくドキュメントといった投影を出力する。ユーザはそれらを確認しながら、ソースを更新し続けることができる。ソースを 1 行直せば、紐付くすべての投影が整合した状態で再生成される ── 何か所も直して回らずに済む。
