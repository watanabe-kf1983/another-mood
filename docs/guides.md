...
`select` を省略すると、各レコードから何も取り出さない（空マップの配列が返る）ので、テンプレートに渡したいフィールドは必ず `select` で明示する。
...
サブテンプレート側（`templates/member.md`）では、`with` で渡したマップのフィールドがそのままトップレベル変数として使える:
...
そこで、Markdown / HTML のような「すでに整形済みの値」は `{ mime_type, content }` 形式のマップ（**Typed Value**）で持っておき、テンプレートでは `.content` を参照する:
