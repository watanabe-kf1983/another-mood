...
メンバー名簿がそのパターン。スキーマの `additionalProperties` の下に値の型を書き、コンテンツファイルは辞書で書く。「同じ形のレコードが何件もある」用途では、ほぼ常にこの **辞書パターン** で書く。スキーマファイル抜粋とコンテンツファイルを再掲:
...
テンプレートからは、**予約名 `prose`** のリストにレコードが現れる:
