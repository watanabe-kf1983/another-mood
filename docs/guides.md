...
**another-mood** は、ソースベース DB のプロセッサで、そういうドキュメント群の整合維持を担うツール。ここでいう **ソースベース DB** とは、ユーザ自身が作成・更新・削除するファイル群 ── 以下「**ソース**」と呼ぶ ── からなるデータベース。ソースは YAML や Markdown 等の形式で、データ・登場人物の型定義・クエリ・ドキュメントテンプレートを含む。ユーザがエディタ等でソースを直接書き換えるのが唯一の DB 操作の手段。一方、another-mood はそれを読み込み、クエリの実行結果・データの一覧・テンプレートに基づくドキュメントなどを出力する。ユーザはそれらを確認しながら、ソースを書き直し続けることができる。ソースを 1 行直せば、紐付くすべての出力が整合した状態で再生成される ── 何か所も直して回らずに済む。
...
[uv](https://docs.astral.sh/uv/) のインストール手順に従って uv を入れたうえで:
...
`__` で始まるディレクトリには、ユーザのテンプレートとは独立にツールが自動生成するページが入る。スキーマ・データ・クエリの現状を書きながら確認するためのもので、[ソースの書き進め方](#ソースの書き進め方) 章で詳述する。
...
サンプルを真似て自分のプロジェクトのソースを書き始めるとき、どこに何を置き、どんな順番で作っていくか、という運用面の話。
...
代償として、スペルミスもエラーにならず黙って空になる。これは、書きながら `__table_view/` でデータの実体を見て、`__meta_query/` でクエリの結果形を確認する書き進め方（[ソースの書き進め方](#ソースの書き進め方) 章「[内側から外側に書く](#内側から外側に書く)」）でカバーする想定。
