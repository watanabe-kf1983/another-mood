...
This page therefore covers installation only for **coding agents** —
agents with built-in filesystem access to your project:

- **Claude Code** (CLI / IDE extensions / Claude Desktop's Code tab)
- **VS Code Copilot Chat** (agent mode)

Chat-style agents (e.g., Claude Desktop's Chat tab, ChatGPT Desktop)
can still call this server's tools, but to run that loop you would
also need a separate filesystem MCP server such as
