...
入力ディレクトリ（schema / contents / queries / templates）は `<project_dir>` を基準に解決される。デフォルト配置は次の通り:

| 種類 | デフォルト | 環境変数 |
|---|---|---|
| スキーマ | `<project_dir>/definition/schema` | `RB_SCHEMA_DIR` |
...
```
warning: skipped (already exists): ./definition/schema/entities.yaml
```
...
1. `<project_dir>/definition/schema` 以下のスキーマを読み込み、`<project_dir>/contents` を正規化
...
入力ディレクトリ（schema / contents / queries / templates）の変更を検知すると、影響のあるステージのみを再実行する。Hugo server はファイル更新を検知してブラウザ側を自動リロードする。`Ctrl+C` で停止する。
...
| `schema_dir` | `<project_dir>/definition/schema` | `RB_SCHEMA_DIR` | — |
