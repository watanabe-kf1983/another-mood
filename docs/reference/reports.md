...
To find the exact values, read the diagnostics. `output/__meta_entity/{entity}.md` shows each record type as a `Type:` heading and each array field's collection type in the attributes table; `output/__meta_query/{query}.md` does the same for query result shapes. Copy the value verbatim.
